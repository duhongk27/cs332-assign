...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.empty
      ...
      val more = for {
        ...
        if !explored.contains(next._1) && !initial.exists(_._1 == next._1)
      } yield next
      val newExplored = explored ++ more.map(_._1)

      initial ++ from(more, newExplored)
    }
  }
   ...
   */
  lazy val pathsToGoal: Stream[(Block, List[Move])] = pathsFromStart.filter { case (block, _) => done(block) }
